...
class DummyTokyoParser(BaseParser):
    prefecture = "東京都"
    region = "関東"
...
def test_run_parser_non_saitama_does_not_fallback_to_osm_import(monkeypatch) -> None:
    parser = DummyTokyoParser()
    logger = logging.getLogger("test")

    monkeypatch.setattr("scraper.fetch", lambda *_args, **_kwargs: None)

    def _unexpected_call(*_args, **_kwargs):
        raise AssertionError("import_new_prefecture should not be called for non-saitama")
